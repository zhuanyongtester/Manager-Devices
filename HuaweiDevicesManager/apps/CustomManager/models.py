...
class UserEvent(models.Model):
    ...
    user_profile  = models.ForeignKey(UserProfile, on_delete=models.CASCADE, verbose_name="用户唯一标识符")
    ...
    def __str__(self):
        return f"{self.user_profile } - {self.event_type} - {self.event_time}"
...
class UserPreferences(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile } - {self.preference_name} = {self.preference_value}"

    class Meta:
        db_table = 't_user_preferences'
        verbose_name = "用户偏好设置"
        verbose_name_plural = "用户偏好设置"
        unique_together = ('user_profile', 'preference_name')  # 确保每个用户的偏好设置唯一
        indexes = [
            models.Index(fields=['user_profile'], name='user_profile_preferences_idx'),  # 修改索引名称
        ]
...
class UserInterests(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile } - {self.interest} ({self.interest_score})"

    class Meta:
        db_table = 't_user_interests'
        verbose_name = "用户兴趣标签"
        verbose_name_plural = "用户兴趣标签"
        unique_together = ('user_profile', 'interest')  # 确保每个用户的兴趣标签唯一
        indexes = [
            models.Index(fields=['user_profile'], name='user_profile_interests_idx'),  # 修改索引名称
        ]


class UserSocial(models.Model):
    user_profile  = models.ForeignKey(UserProfile, on_delete=models.CASCADE, related_name='user_social', verbose_name="用户唯一标识符")
    ...
    def __str__(self):
        return f"{self.user_profile } - {self.relationship_type} - {self.friend_user_id}"

    class Meta:
        db_table = 't_user_social'
        verbose_name = "用户社交关系"
        verbose_name_plural = "用户社交关系"
        unique_together = ('user_profile', 'friend_user_id', 'relationship_type')  # 确保每对用户关系唯一
        indexes = [
            models.Index(fields=['user_profile'], name='user_profile_social_idx'),  # 短索引名称
...
class UserActivity(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile } - {self.activity_type} - {self.activity_time}"
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['user_profile', 'activity_time'], name='user_activity_idx'),  # 组合索引
        ]
...
class UserTokens(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile} - {self.token_type} - {self.access_token[:10]}"
...
class UserSessions(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile} - {self.session_key} - {self.ip_address}"
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['user_profile'], name='user_profile_sessions_idx'),  # 修改索引名称
...
class UserAuthLogs(models.Model):
    ...
    def __str__(self):
        return f"{self.user_profile} - {self.action} - {self.timestamp}"
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['user_profile', 'timestamp'], name='user_auth_logs_combined_idx'),  # 修改索引名称
        ]
