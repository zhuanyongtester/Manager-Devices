...
from ..CustomManager.models import UserProfile
...
class StoresCreateSerializer(serializers.Serializer):
    ...
    def validate_login_id(self, value):
        ...
        if not UserProfile.objects.filter(login_id=value).exists():
            raise serializers.ValidationError("The 'login_id' don't exists.")
        return value
